...
Unit tests (always run). Verify `detectProvider` correctly identifies OpenAI (`sk-`), Vercel (`vck_`), Jina (`jina_`), Ollama (`ollama:model` and `ollama:model@url`), rejects Anthropic (`sk-ant-`) with a helpful message, and rejects unknown prefixes.
