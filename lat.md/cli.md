...
The `agent_end` hook keeps behavior consistent with Cursor/Claude by still using `lat hook cursor stop` for final validation and fallback, while delegating auto-fix execution to the isolated `documenter` subagent (including nested `lat.md/` git repos; see [[cli#hook#Excluding lat.md/ from version control]]).
